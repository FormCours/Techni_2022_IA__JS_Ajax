// V2 - Requete via XML Http Request encapsuler dans une promesse
// **************************************************************
...
weatherFrom.addEventListener('submit', async (event) => {
    ...
    try{
        const qui = await search(weatherUrl.replace('__city__', city));
        displayWeather(qui) 
    }
    catch(error){
        displayError(error)
    ...
    // search(weatherUrl.replace('__city__', city))
    //     .then(displayWeather)
    //     .catch(displayError);
...
// La requete Ajax (=> XMLHttpRequest + Promesse)
const search = (url) => {
    if(!url) {
